...
# -*- coding: utf-8 -*-
"""
图商API直接调用脚本 - Evidence Collection Sub-Skill
版本: 1.5.0
作者: BigPOI Verification System
日期: 2026-03-05

功能说明:
    本脚本专门用于直接调用图商API（高德、百度、腾讯），获取POI相关证据数据。
    绕过内部代理，直接与图商API交互，适用于内部代理不可用的场景。

用途说明:
    该脚本是证据收集技能的图商API直接调用入口，主要作用包括：
    1. 关键词搜索：根据城市和关键词搜索POI
    2. 周边搜索：根据中心点和关键词搜索周边POI
    3. API Key轮询：支持多个API Key的轮询和重试机制
    4. 自动规范化：将搜索结果转换为标准Evidence格式
    5. 错误处理和日志记录

    应用场景：POI证据收集、地图API直接调用、内部代理降级方案
"""
...
def execute(data: Optional[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        # 禁用内部代理，强制直接调用
        config['internal_proxy']['enabled'] = False
...
if __name__ == '__main__':
    if len(sys.argv) < 5:
        print("用法: python call_map_vendor.py <poi_id> <name> <poi_type> <city> [address] [longitude] [latitude]")
        print("示例:")
        print("  关键词搜索: python call_map_vendor.py POI_123 '北京大学第一医院' 090101 '北京市'")
        print("  周边搜索: python call_map_vendor.py POI_123 '北京大学第一医院' 090101 '北京市' '' 116.3723 39.9342")
        sys.exit(1)
