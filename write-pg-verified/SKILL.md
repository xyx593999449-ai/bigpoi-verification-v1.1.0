...
description: 从上游技能生成的本地 JSON 文件中读取大 POI 核实结果并回写 PostgreSQL 成果表。用于执行 poi_verified/poi_init 回库，以及在仅提供 task_id 和 search_directory 时自动查找索引文件；如果同一 task_id 因调度重试产生多个 index 文件，自动按最新时间戳选择最新结果。
---

# write-pg-verified

将上游核实技能产出的 `index.json`、`decision`、`evidence`、`record` 等文件加载后，写入 PostgreSQL 的 `poi_verified` 表，并同步更新 `poi_init.verify_status` 为“已核实”。
...
5. 转换字段后写入 `poi_verified`，并更新 `poi_init`。
...
- 技能保持幂等：`poi_verified` 已存在相同 `task_id` 时，不重复插入，但仍会更新 `poi_init` 状态。
- 日志会记录候选 index 数量和最终选择的最新文件，便于排查重试链路问题。
