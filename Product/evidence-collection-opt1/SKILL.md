...
## Parallel workflow (Sub-agent Mode)
...
3. **使用并行的子任务 (Parallel Sub-agents) 同时执行以下三个分支**。不要等待一个分支运行完再开始另一个，必须确保三个分支的耗时重叠：

- **分支 A (Map Vendor - Main Agent)**：
  运行图商采集与初筛。
  ```bash
  ...
  ```
  完成采集后，立即运行 `write_map_relevance_review.py` 生成初筛结果。

- **分支 B (Web Search - Sub-agent 1)**：
  派生子任务。根据 `web-plan.json` 执行搜索，并将每项结果规范化后写入 `websearch-raw-<timestamp>.json`。

- **分支 C (Web Fetch - Sub-agent 2)**：
  派生子任务。根据 `web-plan.json` 执行网页抓取，并将每项结果规范化后写入 `webfetch-raw-<timestamp>.json`。

4. **汇总与补桩**：
   - 检查 Branch A 的 `missing_vendors`，若有缺失且必要，**并行**启动 `call_map_vendor.py` 补采。
   - 所有并发分支（Map, WebSearch, WebFetch）的文件落地后，执行归并。
