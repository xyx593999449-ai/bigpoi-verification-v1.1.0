...
并返回唯一正式 `evidence_path`。

如需人工介入 review 或自定义 `webfetch`，可继续使用下方分步流程。
...
- `websearch` 分支：根据 `web-plan.json` 中的 `official_sources` 与 `internet_sources` 执行搜索，并把结果保存成 JSON 文件。
  - 必须使用 `websearch_adapter.py` 调内部搜索代理执行查询，固定 provider 顺序 `baidu -> tavily`。
  - 仅当 `baidu` 无结果或失败时才允许回退 `tavily`。
...
`websearch_adapter.py` 推荐调用方式：

```bash
python evidence-collection/scripts/websearch_adapter.py -WebPlanPath <web-plan.json> -OutputPath <output/runs/{run_id}/process/websearch-raw-<timestamp>.json> -PoiId <poi-id> -TaskId <task-id> -RunId <run-id>
```
