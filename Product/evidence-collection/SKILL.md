...
- `websearch` / `webfetch` 分支的原始 JSON 文件或 reviewed JSON 文件
- 缺失图商补采的原始 JSON 文件
...
- 先根据输入 POI 生成本次采集 todo，明确：
  ...
  - 每个节点的输入文件、输出文件和落盘路径
  - `webfetch` 是否需要执行
  - 当 `webfetch` 失败时如何 fallback 到 `websearch-reviewed`
...
### 6. `webfetch` 增强与 fallback

- 若 `websearch-reviewed.json` 中有 `should_fetch=true` 的结果，可运行：

```bash
python evidence-collection/scripts/build_webfetch_plan.py -WebSearchReviewedPath <websearch-reviewed.json> -OutputPath <webfetch-plan.json> -PoiId <poi-id> -TaskId <task-id> -RunId <run-id>
```

- `webfetch` 当前允许由模型 + MCP 自行处理
- 若 `webfetch` 成功，可把结构化结果落为 `webfetch-reviewed.json` 再进入归并
- 若 `webfetch` 失败、超时或不可用，不得阻断主流程，必须继续使用 `websearch-reviewed.json` 进入归并
...
```bash
python evidence-collection/scripts/merge_evidence_collection_outputs.py -PoiPath <input.json> -InternalProxyPath <map-reviewed-internal-proxy.json> -WebSearchPath <websearch-reviewed.json> [-WebFetchPath <webfetch-reviewed.json>] -VendorFallbackPaths <map-reviewed-vendor-a.json> <map-reviewed-vendor-b.json> -OutputPath <output/runs/{run_id}/process/collector-merged.json> -RunId <run-id> -TaskId <task-id>
```
...
- 因为 `webfetch` 失败就放弃已经完成的 `websearch-reviewed`
