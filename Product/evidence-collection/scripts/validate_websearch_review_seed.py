...
def validate_websearch_review_seed_against_catalog(
    ...
    for index, item in enumerate(items):
        ...
        should_fetch = item.get("should_fetch")
        if not isinstance(evidence_ready, bool):
            errors.append(f"items[{index}].evidence_ready must be boolean")
        if not isinstance(should_fetch, bool):
            errors.append(f"items[{index}].should_fetch must be boolean")
        fetch_url = normalize_whitespace(item.get("fetch_url"))
        if should_fetch is True and not fetch_url:
            errors.append(f"items[{index}].fetch_url is required when should_fetch=true")
    ...
    return {
        ...
        "fetch_count": sum(1 for item in item_by_id.values() if item.get("should_fetch") is True),
    }
