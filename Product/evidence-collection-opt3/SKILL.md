...
## Parallel workflow (Programmatic Mode)
...
3. **运行并发主控脚本**。该脚本将利用异步 IO 同时拉起图商采集及相关分支，确保采集效率最大化：

```bash
python evidence-collection/scripts/orchestrate_collection.py -PoiPath <input.json> -RunId <run-id> -TaskId <task-id>
```

4. **处理 Web 分支（Agent 协同）**：
   - 观察 `orchestrate_collection.py` 的输出，确认图商分支状态。
   - **同时**手动/通过指令执行 `websearch` 和 `webfetch` 逻辑，确保它们与主控脚本在时间上重叠。
