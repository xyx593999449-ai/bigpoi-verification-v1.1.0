...
```mermaid
flowchart TD
    A["input.json"] --> B["init_run_context.py"]
    B --> C["evidence-collection"]
    C --> D["write_evidence_output.py"]
    D --> E["verification/write_decision_output.py"]
    E --> F["write_result_bundle.py"]
    F --> G["validate_result_bundle.py"]
    G --> H["write-pg-verified"]
```

## 6. 输出约定

- 正式结果目录：`output/results/{task_id}/`
- 运行隔离目录：`output/runs/{run_id}/process/` 与 `output/runs/{run_id}/staging/`
- 正式结果文件：
